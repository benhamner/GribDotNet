...
let gridDefinitionTemplateTypes = Set.ofList [for g in grib -> g.GridDefinitionSection.GridDefinitionTemplateType]
...
let uWind = (temps |> List.choose (fun t -> match t.ParameterCategory with | Momentum -> (if (t.ParameterNumber = Product.UComponentOfWind) then Some t.ParameterNumber else None) | _ -> None))
uWind.Length
